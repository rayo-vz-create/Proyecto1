...
# --- IMPORTACIONES PARA EL DESCIFRADO AES ---
...
# --- CLAVE MAESTRA ACTUALIZADA (Sincronizada con app.js) ---
# Usamos ljust para rellenar con ceros a la derecha hasta llegar a 32 bytes
RAW_KEY = "ViajeGO_Secure_Key_2026"
CLAVE_AES = RAW_KEY.ljust(32, '0')[:32].encode('utf-8')
...
def descifrar_datos(data_encriptada):
    try:
        if not data_encriptada:
            return {}
        raw = base64.b64decode(data_encriptada)
        # Forzamos modo ECB para coincidir con la implementación estándar de CryptoJS simple
        cipher = AES.new(CLAVE_AES, AES.MODE_ECB)
        decrypted_raw = cipher.decrypt(raw)
        # Aplicamos unpad para limpiar el bloque de 16 bytes
        decrypted_str = unpad(decrypted_raw, AES.block_size).decode('utf-8')
        
        datos = json.loads(decrypted_str)
...
def encriptar_respuesta(data_dict):
    try:
        texto_plano = json.dumps(data_dict).encode('utf-8')
        cipher = AES.new(CLAVE_AES, AES.MODE_ECB)
        # Agregamos padding antes de encriptar para evitar errores de tamaño de bloque
        encriptado = cipher.encrypt(pad(texto_plano, AES.block_size))
        return base64.b64encode(encriptado).decode('utf-8')
    except Exception as e:
        print(f"❌ Error encriptando respuesta: {e}")
...
# --- NUEVO: DECORADOR PARA PROTECCIÓN CSRF ---
def csrf_protect(f):
    @wraps(f)
    def decorated_function(*args, **kwargs):
        # Extraemos el token del encabezado que configuramos en app.js
        token_recibido = request.headers.get('X-CSRF-Token')
        
        # Si el token no existe, bloqueamos la petición
        if not token_recibido:
            print("🚨 [BLOQUEO CSRF]: Petición rechazada por falta de Token.")
            return jsonify({
                "success": False, 
                "message": "Error de seguridad: Token CSRF faltante."
            }), 403 
            
        print(f"🛡️ [CSRF VALIDADO]: Petición autorizada.")
        return f(*args, **kwargs)
    return decorated_function

# ==================== RUTAS: VUELOS, HOTELES Y AUTOBUSES ====================
...
def vuelo_item(id):
    if request.method == 'DELETE':
        ...
        return jsonify({"success": True, "message": "Vuelo eliminado"})
...
def hotel_item(id):
    if request.method == 'DELETE':
        ...
        return jsonify({"success": True, "message": "Hotel eliminado"})
...
def handle_user_registration():
    d = request.json
    if d and 'encrypted_data' in d:
        d = descifrar_datos(d['encrypted_data'])
    conn = get_db_connection(); cursor = conn.cursor()
    try:
...
def login():
    ...
    if user and bcrypt.check_password_hash(user['password_hash'], d.get('password')):
        ...
        print(f"\n TOKEN JWT ({user['nombre']}):\n{token}\n\n🛡️  CSRF TOKEN GENERADO:\n{csrf_token}\n")
        print(f"  CSRF TOKEN GENERADO: {csrf_token}\n", flush=True)
        ...
        payload = {
            ...
            "user": fix_encoding(user)
        }
        
        return jsonify({"encrypted_response": encriptar_respuesta(payload)})
...
def verify_2fa():
    ...
    user = cursor.fetchone(); conn.close()
    if not user or not user['secret_2fa']: return jsonify({"success": False, "message": "Error en 2FA"}), 401
...
def listar_agencias():
    ...
    res = cursor.fetchall(); conn.close(); return jsonify(fix_encoding(res))
...
@csrf_protect
def crear_reserva():
    print(f"\n--- 🔍 DEPURACIÓN DE RESERVA ---")
    d = request.json
    print(f"📥 Datos en bruto recibidos: {d}")

    if d and 'encrypted_data' in d:
        d_descifrado = descifrar_datos(d['encrypted_data'])
        print(f"🔓 Datos descifrados: {d_descifrado}")
        d = d_descifrado 
    
    if not d or 'user_id' not in d:
        print("❌ ERROR: Los datos están vacíos o el descifrado falló.")
        return jsonify({"success": False, "message": "Datos de reserva inválidos o corruptos"}), 400

    conn = get_db_connection(); cursor = conn.cursor()
    try:
        code = f"RES-{random.randint(10000, 99999)}"
        dets = json.dumps(d.get('details', {})) 
        
        sql = """INSERT INTO reservas 
                 (user_id, reservation_code, service_type, item_name, date_start, date_end, num_guests, details_json, total_price) 
        ...
        conn.commit()
        print(f"✅ RESERVA CREADA EXITOSAMENTE: {code}")
...
def generate_2fa():
    try:
        ...
        totp_auth_url = pyotp.totp.TOTP(secret).provisioning_uri(name=user[0], issuer_name="ViajeGO")
